...
# Table Token
module "dynamodb_table_token_eu_south_1" {
...
## Table Token replica
module "dynamodb_table_token_eu_central_1" {
  source = "terraform-aws-modules/dynamodb-table/aws"
  ...
  server_side_encryption_enabled     = true
  server_side_encryption_kms_key_arn = aws_kms_alias.dynamo_db_replica[0].target_key_arn
...
resource "aws_dynamodb_global_table" "token" {
  count = var.dynamodb_region_replication_enable ? 1 : 0
