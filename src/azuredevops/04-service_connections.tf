...
resource "azuredevops_serviceendpoint_github" "github_ro" {
  ...
  auth_personal {
    personal_access_token = jsondecode(data.aws_secretsmanager_secret_version.devops.secret_string)["github_ro_token"]
  }
  lifecycle {
...
resource "azuredevops_serviceendpoint_github" "github_pr" {
  ...
  auth_personal {
    personal_access_token = jsondecode(data.aws_secretsmanager_secret_version.devops.secret_string)["github_pr_token"]
  }
  lifecycle {
    ignore_changes = [description, authorization]
  }
